...
it('throws TransportException when to recipient list is empty', function () {
    ...
    $transport->send($email);
})->throws(TransportException::class, 'Brevo requires at least one "to" recipient.');

it('falls back to noreply@example.com when sender from is missing', function () {
    Http::fake([
        'https://api.brevo.com/v3/smtp/email' => Http::response(['messageId' => 'brevo_noreply_123'], 201),
    ]);
    ...
    Http::assertSent(function (Request $request) {
        ...
        return $data['sender']['email'] === 'noreply@example.com';
    });
});
