...
it('registers publishable config with tag mailer-config', function () {
    $publishes = LaravelMailerServiceProvider::pathsToPublish(null, 'mailer-config');

    expect($publishes)->not->toBeEmpty();

    $sourceConfig = array_key_first($publishes);
    expect($sourceConfig)->toBeString()
        ->and(file_exists($sourceConfig))->toBeTrue()
        ->and($publishes[$sourceConfig])->toBe(config_path('mailers.php'));
});
